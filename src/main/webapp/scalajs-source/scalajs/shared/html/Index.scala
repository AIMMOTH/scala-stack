...
object Index {
  ...
  def apply(css: Stylisch, minified: Boolean = false, language : Languages.Language = Languages.default) = {
    ...
    val (postFunction, getFunction) = classOf[scalajs.jquery.AjaxRest] match {
      case restClass =>
        (methodPath(restClass, "doPost"), methodPath(restClass, "doGet"))
      }
    val documentReady = methodPath(classOf[scalajs.jquery.Document], "ready") // scalajs.Document().ready()
